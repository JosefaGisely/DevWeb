...
public class UsuarioDao {
    ...
    @Override
    public List<Usuario> listaTodos(List u){
        try {
    ...
    public Usuario busca(int id){
        try {
            // Cria o statment que contém a Query de consulta
            PreparedStatement stmt = this.conexao.prepareStatement("SELECT * FROM usuario WHERE idUsuario = ?");
            stmt.setInt(1, id);
            // Cria uma varíavel para receber o resultado da Query
            ResultSet rs = stmt.executeQuery();
    ...
    public Usuario buscaLogin(String email, String senha){
        try {
            // Cria o statment que contém a Query de consulta
            PreparedStatement stmt = this.conexao.prepareStatement("SELECT * FROM usuario WHERE "
                    + "	email = ? and "
                    + "senha = ?");
    ...
    public void altera(Usuario u){
        try {
            PreparedStatement stmt = this.conexao.prepareStatement("UPDATE usuario "
                    + "set cpf = ?,"
                    + "nomeCompleto = ?, "
                    + "email = ?, "
                    + "celular = ?, "
    ...
    public void alteraSenha(Usuario u){
        try {
            PreparedStatement stmt = this.conexao.prepareStatement("UPDATE usuario SET "
                    + "senha = ? "
                    + "WHERE idUsuario = ?");
    ...
    public void remove(int id){
        try {
            ...
            PreparedStatement stmt = this.conexao.prepareStatement("DELETE FROM usuario WHERE idUsuario = ?");
            stmt.setInt(1, id);
            // Executa o Statment
            stmt.execute();
            // Encerra o Statment
